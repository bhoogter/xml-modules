...
//print "<br/>remove_source($id)";
	...
	function count_iobj()		{	return is_array($this->iOBJa)?count($this->iOBJa):0;	}
	function add_iobj($i)		{	if (!$this->count_iobj()) $this->iOBJa = array(); array_push($this->iOBJa, $i); return true;	}
	function remove_iobj($i)		{	if ( $this->count_iobj()) {foreach($this->iOBJa as $a=>$b) if ($i->gid() == $b->gid()) {unset($this->iOBJa[$a]);break;}}	}
...
print "<br/>iOBJ Array:";print_r($this->iOBJa);
		foreach($this->iOBJa as $b)	print "<br/>".(++$n).": ".(method_exists($b, 'gid')?$b->gid():'UNKNOWN('.get_class($b).')');
		}
	function iOBJ()			{	if (!$this->count_iobj()) $this->iOBJa = array(); $x = array_pop($this->iOBJa);array_push($this->iOBJa, $x); return $x;	}
	function iOBJ2()			{	if (!$this->count_iobj()) $this->iOBJa = array(); $x = array_pop($this->iOBJa);$y = array_pop($this->iOBJa);if ($y) array_push($this->iOBJa, $y);if ($x) array_push($this->iOBJa, $x); return $y;	}
	function result()			{	return ($x=$this->iOBJ())?$x->result:null;	}
	function result2()			{	return ($x=$this->iOBJ2())?$x->result:null;	}
	function resultDoc()			{	return ($x = $this->result())?$x->Doc:null;	}
	function resultDoc2()		{	return ($x = $this->result2())?$x->Doc:null;	}
...
//print "<br/>SOURCE::cnt: ID=$id, Path=$p";
	...
	function FetchPagePart($PName, $El)
		{
		if ($PName=="") {$this->backtrace();return die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchPagePart</u> El=$El");}
		if ($El   =="") {$this->backtrace();return die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No El in FetchPagePart</u>");}
	...
	function ObjectList() {return $this->lst("//SYS/*/zobjectdef/@name");}
	function FetchDSPart($ZID, $El)
		{
//print "<br/>FetchDSPart($ZID, $El)";
		if ($ZID=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZID in FetchDSPart</u> El=$El");}
		return $this->get("//SYS/*/datasource[@id='$ZID']/$El");
		}
	function FetchObjPart($ZName, $El)
		{
//print "<br/>FetchObjPart($ZName, $El)";
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjPart</u> El=$El");}
...
//print "<br/>FetchObjFieldPart($ZName, $fid, $El)";
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjFieldPart</u>, fid=$fid, El=$El");}
		if ($fid=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No fid in FetchObjFieldPart</u>, ZName=$ZName, El=$El");}
		$t = $this->get("//SYS//zobjectdef[@name='$ZName']/fielddefs/fielddef[@id='$fid']/$El");
		return $t;
		}
	function FetchObjDefString($ZName)
		{
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjDefString</u>");}
		return $this->def("//SYS//zobjectdef[@name='$ZName']");
		}
	function FetchObjFields($ZName)
		{
//print "<br/>FetchObjFields($ZName)";
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjFields</u>");}
		return $this->lst("//SYS//zobjectdef[@name='$ZName']/fielddefs/fielddef/@id");
		}
	function FetchObjFieldDefault($ZName, $fid)
		{
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjFieldDefault</u>");}
		if ($fid=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No fid in FetchObjFieldDefault</u>, ZName=$ZName");}
		$d = FetchObjFieldPart($ZName, $fid, "@default");
		return $this->php_hook($d);
		}
	function FetchObjFieldCategories($ZName)
		{
		if ($ZName=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No ZName in FetchObjFieldCategories</u>");}
...
//print "<br/>FetchActPart($Act, $El)";
		if ($Act=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No Act in FetchActPart</u>");}
...
//print "<br/>FetchActRulePart($Act, $R, $El)";
		if ($Act=="") {$this->backtrace();die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No Act in FetchActRulePart($Act, $R, $El)</u>");}
...
//	if ($T=="") die("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No data-type in FetchDTPart</u>");
	...
	public static function FetchSpecPart($Module, $El)
		{
		if ($Module=="") {juniper()->backtrace("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No Module in FetchSpecPart</u> El=$El");}
		if ($El   =="") {juniper()->backtrace("<br/><font style='font-weight:bold;font-size:20'>DIE:</font> <u>No El in FetchSpecPart</u>");}
//print "<br/>juniper_module::FetchSpecPart($Module, $El), Q: //SYS/*/module-specification[@name='$Module']/$El";
	...
	function include_styles()				{	$this->include_support_files('', 'css');	}
	function include_scripts()				{	$this->include_support_files('', 'js');	}
...
//print "<br/>SUPPORT FILE: including PHP file: ".$f;
				...
				case 'css':
					...
					break;
				case 'js':
					if ($d!="") $da = explode($d, ","); else $da= array();
...
//						wp_register_script( $itm, $F, $da);
	...
	function get_option($f)
		...
		switch(strtoupper($f))
			{
			case "WP":	return defined("WP_CONTENT_DIR") ? "1" : "0";
			case "ADMIN": return YesNoVal(juniper_querystring::get_querystring_var("_ADMIN"),false)?"1":"0";
			case "SUBZ":  return YesNoVal(juniper_querystring::get_querystring_var("_ADMIN"),false)?"1":"0";
			default:	return "";
			}
		}
///////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
	...
	function is_php_hook($f) {return is_string($f) && substr($f, 0, 4)=="php:";}
	function php_hook_has_args($f) {return $this->is_php_hook($f) && strpos($f,",")!==false;}
		...
		if (count($S) > 1)
			...
			foreach($S as $ss)
				...
				if ($ss[0]!="@") $v = $ss;
					...
					{
					$sst = substr($ss, 1);
					if (is_array($args)) $v = $args[$sst]; else $v = juniper_querystring::get_querystring_var($args, $sst);
					}
				$a[] = $v;
				}
			$args = $a;
...
//print "<br/>InterpretFields: $f";
	...
	function log_request($type="")
		...
		switch($type)
			{
			case "ajax":	$f = $f = juniper_dir("/_request-ajax.adj");break;
			default:	$f = $f = juniper_dir("/_request.adj");break;
			}
...
//print "<br/>;lkj;lj.........";
	...
	function HandledElementsArray()	{return $this->lst("//SYS/*/module-specification/components/element/@name");}
	function HandledElements()		{return ",".implode(",",$this->HandledElementsArray()).",";}
	function ElementIsHandled($e)	{return in_array($e, $this->HandledElementsArray());}
	function PerfectElementsArray()	{return $this->lst("//SYS/*/module-specification/components/perfect/@name");}
	function PerfectElements()		{return ",".implode(",",$this->PerfectElementsArray()).",";}
	function ElementIsPerfect($e)	{return in_array($e, $this->PerfectElementsArray());}
...
//print "<br/>Target=$Target";die();
	...
	function ajaxToken($a)
		{
		if ($this->AJAX) $c = $this->AJAXreferer; else $c = juniper_current_page();
...
//print "<br/>render=$render";
	...
	private function timedkey_timer($duration=12)	{return intval(time() / (3600));}
	private function timedkey_length()			{return 10;}
	private function timedkey_part($hash)			{return substr($hash.$hash.$hash.$hash, strlen($hash)*3 - $this->salt_numeral($hash), $this->timedkey_length());}
...
function zoGet($p)		{	return juniper()->get($p);		}
function zoSet($p, $v)	{	return juniper()->set($p, $v);	}
function zoCnt($p)		{	return juniper()->cnt($p);		}
function zoLst($p)		{	return juniper()->lst($p);		}
function iOBJ() {return juniper()->iOBJ();}
function iOBJ2() {return juniper()->iOBJ2();}
